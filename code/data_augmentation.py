...
def train_split(test_size = 0.20):
    ...
    x_train_o_temp, x_test_o, y_train_o_temp, y_test_o = train_test_split(skin_df['path'], skin_df['cell_type_idx'], test_size=test_size, random_state=1234)

    x_train_o, x_val_o, y_train_o, y_val_o = train_test_split(x_train_o_temp, y_train_o_temp, test_size=0.10, random_state=1234)
    
    directory_list = ['training_set', 'test_set', 'validation_set']
    for directory in directory_list:
    ...
    for element in x_val_o:
        image = element[len(base_skin_dir)+1:]
        shutil.copyfile(base_skin_dir + '/' + image, directory_list[2] + '/' + image)
...
def test_metadata(ham_metadata):
    metadata = {}
    for file_ in os.listdir('test_set'):
        filename_tuple = os.path.splitext(file_)
        filename = filename_tuple[0] + filename_tuple[1]
...
def validation_metadata(ham_metadata):
    metadata = {}
    for file_ in os.listdir('validation_set'):
        filename_tuple = os.path.splitext(file_)
        filename = filename_tuple[0] + filename_tuple[1]
        metadata[filename] = ham_metadata[filename_tuple[0]]

    with open('val.csv', 'w', newline='') as csv_file:
        writer = csv.writer(csv_file)
        writer.writerow(['image_id', 'dx'])
        for key, value in metadata.items():
            writer.writerow([key, value])
